-- awake_transport: time changes with Link/MIDI transport start-stop
-- based on awake 2.6.0 @tehn
-- transport mod for Ableton Link Start/Stop Sync
-- l.llllllll.co/awake
--
...
alt = false
-- Transport state. Start stopped so Ableton/Norns transport decides when the sequence runs.
running = false
...
function start()
  -- Reset to 0 so the next clock tick advances to step 1.
  reset()
  running = true
  if g then gridredraw() end
  redraw()
...
function redraw()
  ...
  screen.level(running and 15 or 2)
  screen.move(92,10)
